...
class Application {
    constructor() {
        ...
        this.eclipseView = null;
        this.instrumentAnalyzer = null;
        this.variableAnalyzer = null;
        this.starmapGenerator = null;
    ...
    async _init() {
        ...
        try {
            ...
            // 4. 初始化日食视图 (EclipseView)
            this.eclipseView = new EclipseView();
            window.eclipseView = this.eclipseView;

            // 5. 初始化仪器误差分析 (InstrumentAnalyzer)
            this.instrumentAnalyzer = new InstrumentAnalyzer();
            window.instrumentAnalyzer = this.instrumentAnalyzer;

            // 6. 初始化变星分析 (VariableStarAnalyzer)
            this.variableAnalyzer = new VariableStarAnalyzer();
            window.variableAnalyzer = this.variableAnalyzer;

            // 7. 初始化星图生成器 (StarmapGenerator)
            this.starmapGenerator = new StarmapGenerator();
            window.starmapGenerator = this.starmapGenerator;
    ...
    _bindModuleEvents() {
        ...
        // EclipseView → UI：日食选中 → 显示详情
        this.eclipseView.onEclipseSelected = (eclipse) => {
            this.ui.showEclipseDetail(eclipse);
        };

        // InstrumentAnalyzer → UI：仪器选中 → 显示详情
        this.instrumentAnalyzer.onInstrumentSelected = (instrument) => {
            this.ui.showInstrumentDetail(instrument);
        };

        // VariableStarAnalyzer → UI：变星选中 → 显示详情
        this.variableAnalyzer.onVariableSelected = (variable) => {
            this.ui.showVariableDetail(variable);
        };

        // StarmapGenerator → UI：星图生成 → 渲染结果
        this.starmapGenerator.onStarmapGenerated = (result) => {
            this.ui.showStarmapResult(result);
        };

        // UI → 各模块：功能tab切换 → 显示/隐藏对应面板
        window.onFeatureTabChange = (featureName) => {
            this._handleFeatureTabChange(featureName);
        };
    }
    ...
    async _loadEclipses() {
        try {
            const list = await window.api.getEclipses();
            this.eclipseRecords = list || [];
            this.eclipseView.records = this.eclipseRecords;
        } catch (e) { console.warn('日食记录加载失败:', e); }
    }
    ...
    async _loadInstruments() {
        try {
            ...
            this.instrumentAnalyzer.instruments = this.instruments;
        } catch (e) { console.warn('仪器列表加载失败:', e); }
    }

    async _loadVariables() {
        try {
            const list = await window.api.getVariables();
            this.variableStars = list || [];
            this.variableAnalyzer.variables = this.variableStars;
        } catch (e) { console.warn('变星列表加载失败:', e); }
    }

    _handleFeatureTabChange(featureName) {
        if (!featureName || featureName === 'stars') {
            this.ui.toggleFeaturePanel(null);
            return;
        }
        this.ui.toggleFeaturePanel(featureName);
    }
