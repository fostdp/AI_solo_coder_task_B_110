...
mod tests {
    ...
    fn test_lomb_scargle_known_period() {
        ...
        assert!(
            rel_err < 0.20,
            "Period detection rel err {} should be < 20% (true={}, detected={})",
    ...
    fn test_period_change_linear_drift() {
        ...
        assert!(
            pdot > 0.0,
            "pdot should be positive (P increasing), got {}",
            pdot
        );
        assert!(
            delta_p.unwrap_or(0.0) > 0.0,
            "period delta should be positive"
        );
        assert!(
            sigma > 0.5,
    ...
    fn test_uniform_sampling_gap() {
        ...
        assert!(max_gap > 100.0, "Should have a large gap ~1000yr, got {}", max_gap);
